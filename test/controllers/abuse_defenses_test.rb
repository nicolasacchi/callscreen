...
class AbuseDefensesTest < ActionDispatch::IntegrationTest
  ...
  test "auto-blacklist: 3rd spam in 7d flips contact.blacklisted to true + audit row" do
    contact = @tenant.contacts.create!(phone: CALLER_FROM)
    # Two prior spam calls in window
    2.times do |i|
      @tenant.calls.create!(
    ...
    # Trigger a screen.gather_ended that classifies the current call as
    # high-confidence spam — the third occurrence within the window.
    @tenant.calls.create!(
      call_sid: CCID, call_control_id: CCID,
      from_number: CALLER_FROM, to_number: TENANT_TO,
      status: :screening, flow_state: "awaiting_speech",
      contact: contact
    )
    ...
    contact.reload
    assert contact.blacklisted, "contact should be auto-blacklisted after the 3rd spam"
  ...
  test "auto-blacklist: does not fire below threshold" do
    contact = @tenant.contacts.create!(phone: CALLER_FROM)
    # Only one prior spam → current would be the 2nd, threshold is 3.
    @tenant.calls.create!(
    ...
    assert_no_difference -> { AuditLog.where(action: "auto_blacklist").count } do
      post_event gather_payload(transcript: "warranty plan")
    end
    contact.reload
  ...
  test "auto-blacklist: ignores spam outside the window" do
    contact = @tenant.contacts.create!(phone: CALLER_FROM)
    # 3 prior spam BUT all outside the 7-day window
    3.times do |i|
      @tenant.calls.create!(
    ...
    assert_no_difference -> { AuditLog.where(action: "auto_blacklist").count } do
      post_event gather_payload(transcript: "scam!")
    end
  ...
  test "auto-blacklist: skips already-whitelisted contacts" do
    ...
    assert_no_difference -> { AuditLog.where(action: "auto_blacklist").count } do
      post_event gather_payload(transcript: "warranty")
    end

    contact.reload
    refute contact.blacklisted, "operator-whitelisted contact must never be auto-blacklisted"
    assert contact.whitelisted, "whitelist flag preserved"
  end
  ...
  test "auto-blacklist: scoped per-tenant — same caller blacklisted on A is NOT blacklisted on B" do
    other = tenants(:other)
    other.update!(auto_blacklist_threshold: 1, auto_blacklist_window_days: 7)

    # Trigger auto-blacklist for the OTHER tenant only.
    other_contact = other.contacts.create!(phone: CALLER_FROM)
    other.calls.create!(
      call_sid: CCID, call_control_id: CCID,
      from_number: CALLER_FROM, to_number: "+390591111111",  # we'll route to other via dedicated_number
      status: :screening, flow_state: "awaiting_speech",
    ...
    assert other_contact.blacklisted, "other tenant's contact got auto-blacklisted"

    # Default tenant's same-phone contact is untouched
    @tenant.contacts.create!(phone: CALLER_FROM, blacklisted: false)
    assert_not @tenant.contacts.find_by(phone: CALLER_FROM).blacklisted
  end
